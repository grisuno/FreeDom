...
$(BUILD_DIR)/test_interp: $(TEST_DIR)/test_interp.c $(BUILD_DIR)/interp.o | $(BUILD_DIR)
	$(CC) $(CFLAGS) $(CMOCKA_CFLAGS) $^ -o $@ $(LDFLAGS) -lm $(CMOCKA_LIBS)
...
# `make drift` -- IPC wire drift lock (spec/ipc_codec.md). Fails build when
# write_view/read_view array widths differ from TAB_WIRE_* constants.
drift:
