...
- `unset` (line 449) `* background paints as if unset (no border/box-shadow-style
                 * "broken image" pla...`
- `rd_free` (line 478) `void rd_free(rd_doc *d)`
- `rd_count` (line 493) `size_t rd_count(const rd_doc *d)`
- `rd_at` (line 497) `const rd_block *rd_at(const rd_doc *d, size_t i)`
- `rd_box_count` (line 502) `size_t rd_box_count(const rd_doc *d)`
- `rd_box_at` (line 506) `const pv_box_def *rd_box_at(const rd_doc *d, size_t i)`
- `rd_kind_name` (line 511) `const char *rd_kind_name(rd_kind k)`
- `rd_block_tag` (line 524) `const char *rd_block_tag(const rd_block *b)`
- `rd_input_label` (line 554) `const char *rd_input_label(int input_type)`
- `rd_image_label` (line 572) `const char *rd_image_label(rdp_img_decision d)`
- `rd_image_fail_label` (line 583) `const char *rd_image_fail_label(img_fail_reason reason)`
