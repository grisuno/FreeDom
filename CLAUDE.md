...
medición (ago 10, `TOTAL 210.59`): `jkanime` 54.21, `wikipedia` 47.16, `ddg-results` 33.30,
`slashdot` 25.57, `hackernews` 20.25, `float-beside` 17.06, `table-fit` 7.09, `rem-62` 5.95.
El término dominante del score es `h_ratio`, así que **lo que apila verticalmente lo que debería
ir al lado es siempre el bug más caro.**

| # | Hito | Estado | Esfuerzo |
| :-- | :-- | :-- | :-- |
| R12 | **`jkanime` (54.2) y `wikipedia` (43.8) siguen siendo las dos peores. MEDIDO (ago 10):** Wikipedia era `@media` a 1920 activando el layout fijado ⇒ artículo en 248 px; **arreglado** por `@media` al ancho de render (tanda 12) → artículo ancho, 47.2→43.8. **Falta:** (a) la barra `columnEnd` de Vector queda en ~15 px con texto de 1 char — sus hijos TOC son `display:none` por selector **compuesto** aún no aplicado, y falta placement por `grid-template-areas` (los ítems van round-robin `índice % cols`, no por nombre de área); (b) listas de referencias multi-columna (`column-width:30em`) en 1 columna (CSS multi-col no soportado). **jkanime** es Bootstrap **flex**, no grid: el alto extra es la barra "ANIMES RECIENTES" (~40 tarjetas media-object que apilan vertical en vez de miniatura+texto al lado; imágenes remotas rotas offline inflan la brecha). `[[freedom-parity-container-diagnosis-2026-08-10]]` | Parcial | Alto |
