...
- **Hito 20e parte 2 — JS vivo: lo que falta.** Cerrados: ejecución/DOM escribible/eventos-timers
  sintéticos/`innerHTML`/`location`+nav/clic (Ver `[[freedom-live-js]]`, `[[freedom-box-engine-and-dispatcher]]`)
  y scripts externos (Ver `[[freedom-external-scripts]]`). Falta: **timers async reales** (event loop
  en el worker, no solo `__fireDeferred` por rondas), getter de `innerHTML` (serialización), `defer`/
...
- **Hito 22b — Descargas/zoom: pulido.** Base cerrada (Ver `[[freedom-zoom-download]]`). Falta: ícono
  de recarga en toolbar, barra de progreso + descarga async, anti-clobber (`(1)`), zoom por-sitio
  persistido (depende del Hito 10).
- **Hito 10 — Persistencia de preferencias.** Nada persiste hoy entre sesiones (imágenes/CSS/tema/
  Tor-I2P/zoom son env o por-sesión); falta cifrar con `local_store`/`disk_store` + Argon2id.
...
- `<pre>`: **gap nuevo encontrado** (el skull ASCII-art de LazyOwn se ve amontonado): `<pre>` es
  `BLOCK` en `box_style.c` pero no tiene UA-default `white-space:pre` (el texto se colapsa como
  cualquier párrafo) ni fuente monoespaciada. Requiere dividir el texto de un `<pre>` en una línea
  por `\n` de origen (cada una con `block_break`) en vez de colapsar espacios — tratable pero no
  trivial (toca el loop principal de `page_view.c`); diferido a un hito propio, no forzado aquí.
