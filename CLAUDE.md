...
medición (ago 14, tanda 19, `TOTAL 129.19` sobre **17** páginas; el corpus viejo cayó de 154.06 a
126.04): `jkanime` 55.19, `wikipedia` 28.87, `slashdot` 16.63, `ddg-results` 12.59,
`hackernews` 3.60, `grid-areas` 2.22, `table-fit` 1.64, `em-units` 1.30, `inline-replaced` 1.15,
`ua-metrics` 0.98, `margin-collapse` 0.93, `inline-links` 0.92, `css-table` 0.78,
`float-beside` 0.74, `pct-box` 0.68, `rem-62` 0.54, `multicol` 0.43. El término dominante del
score es `h_ratio`, así que **lo que apila verticalmente lo que debería ir al lado es siempre el
bug más caro** — salvo en `slashdot`, cuya `h_ratio` ya es 1.002 y cuyo score lo manda entero el
`col_mae` (le falta la columna principal + rail).

**`jkanime` (55.19) NO es una medición del motor y no se debe optimizar contra ella.** Sus 115
imágenes vienen de un CDN de terceros; **Firefox las descarga** en el arnés y Freedom las bloquea
por diseño (Privacy by Default), así que los dos motores están midiendo documentos distintos:
Firefox pinta miniaturas de ~110 px y Freedom fluye el texto `alt` de cada una. La medición limpia
más cara es **`wikipedia` (28.87)**.

**Brechas nuevas, medidas y sin arreglar (ago 14).** (a) **El fondo de un wrapper se
pinta por FILA, no como caja continua:** `css_has_boxdeco` excluye
`background-color` a propósito (el fondo viaja en la banda de la fila), así que un
`<div class=wrap style=background:#eee>` con dos `<p>` adentro sale como dos bandas
grises con un hueco blanco entre ellas, donde Firefox pinta un rectángulo continuo
(el margen es transparente y deja ver el fondo del PADRE). Medido en
`tests/parity/pages/margin-collapse.html` sección D. Arreglarlo es agregar el fondo a
`css_has_boxdeco`, lo que **multiplica el número de cajas** — hay que medir el costo
contra `PV_MAX_BOXES` antes. (b) **`slashdot`: la columna principal + rail.**
`.container > .main-wrap` y `.rail-right` son ambos `float:left` y
`.main-wrap.has-rail-right > .main-content` reserva el hueco con
`margin-right:320px`; el motor pinta el artículo a ancho completo y el sidebar no cae
al lado. Es el `col_mae` 0.395 que domina su score. (c) **`examples/position-overlay`
exporta 30000 px** (el tope `PNG_MAX_H`) — preexistente, alguna caja fija se mide
contra el `vp_h` de 30000.
...
**Cuidado al elegir el próximo objetivo:** `jkanime` es la peor por score y **no es una
medición limpia del motor**. Sus imágenes vienen de un CDN de terceros que la política de red
bloquea por diseño, así que Firefox las pinta y Freedom no; el residual real es
`grid-template-areas` y el sobre-ancho por porcentajes en ítems de grilla. La medición limpia
que queda es **`wikipedia` (39.36)**: le falta colocación de grid por nombre/línea y el resto de
la maquetación de `display:table` (una tabla real de N columnas, no la fila sintetizada).

**Lección de las tandas 13, 16 y 17, vale para elegir el próximo objetivo:** los bugs más caros
