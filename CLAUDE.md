...
| Imágenes | `image_decode` (`img_`), `data_url` (`du_`) | Decodificado **PNG + JPEG + WebP + GIF estático dentro del worker confinado**; topes anti-DoS; salida ARGB lista para Cairo. JPEG es excepción de doctrina autorizada (libjpeg con fuente en memoria + `longjmp` que nunca llama `exit()`); GIF usa un decoder LZW **propio** en C puro (sin giflib — menos superficie, fuzzeado). `data_url` (puro, fuzzeado) parsea `data:` URIs y decodifica base64 — el padre solo recorta el payload (sin decodificar), el worker hace el decode base64 + de formato, igual que cualquier imagen fetcheada. |
...
- **Fix (2026-07-15) — Export headless con imágenes reales.** `ui_render_png`/`ui_render_pdf` usaban un `browser_window` en cero **sin worker** ⇒ NUNCA decodificaban imágenes: `--images` solo cambiaba la decisión (BLOCK→ALLOW), así que salía el placeholder engañoso "image (allowed): nombre" para toda imagen (y el comentario que prometía "actually shows images" era falso). Nuevas `ui_render_png_images`/`ui_render_pdf_images` (`gui/browser_ui.c`) decodifican las imágenes permitidas por el **worker confinado** (locales de disco confinadas al dir del doc, remotas por el fetcher bajo política — los bytes hostiles nunca tocan el proceso confiable). DRY: `load_images` refactorizada para tomar un `tab_fetch_fn` (GUI pasa `gui_image_fetch`, headless `headless_fetch`). `run_headless` ahora arma origen `file://realpath` para archivos locales (antes NULL ⇒ "invalid URL"), como el GUI. Con `--images` el `/visual-review` **por fin verifica imágenes** (antes imposible headless); un decode que falla cae a su fail-label honesto ("image (unsupported format)"), no al engañoso "allowed". Test E2E `test_download_png_images_local` + ASan limpio (subproceso freedom instrumentado). Ver `[[freedom-visual-review-headless]]`, `spec/freedom.md`.
- **Hito M1.1 (2026-07-15, modernización Fase 1, incremento 1/N — fundación del compositor).** Módulo **puro** `compositor` (`cx_`, `include/compositor.h`/`src/compositor.c`/`spec/compositor.md`): fuente única del **contexto de apilamiento** (`cx_forms_stacking_context`: opacity<1 / mix-blend≠normal / isolation:isolate / relative·absolute+z-index / fixed·sticky) y del **orden de pintado de CSS 2.1 App E** (`cx_box_layer` → 7 capas; `cx_item_compare`/`cx_sort` estable = orden total layer→z-index→documento). Sin I/O, sin Cairo, sin GPU, sin asignar surfaces — lógica de decisión testeable directamente como `js_policy`/`webcaps` (18 tests, ASan/UBSan limpio, sin fuzz: no parsea contenido remoto). Ver `[[freedom-modernization-roadmap]]`, `spec/compositor.md`.
- **Hito M1.1 (2026-07-16, incremento 2/N — `cx_sort` cablea `bt_resolve_positioning`).** `src/box_tree.c` reemplaza el insertion-sort ad-hoc `(z_index ASC, doc_order ASC)` de Stage 2 por `cx_item`/`cx_sort`: por cada caja posicionada se arma un `cx_style` (position/z_index/z_auto reales; opacity/mix_blend/isolation en su default "sin trigger" porque `pv_box_def` todavía no los carga) y `cx_box_layer` decide su capa (siempre NEG_Z/ZERO_Z/POS_Z para este subconjunto, ya que todo lo que entra aquí está posicionado). **Byte-idéntico verificado**: PNG headless de una página con 4 cajas `position:absolute` en z-indices -1/0/1/5 es `cmp`-idéntico antes/después (`git stash` del cambio + rebuild + comparación binaria), y los 34 tests de `test_box_tree` (incluidos `test_positioning_stacking_order`/`_doc_order_tiebreak`) + suite completa + ASan/UBSan pasan limpios. Primer *caller* real de `cx_*` (antes solo enlazaba sin uso); `compositor.o` ahora es dependencia de link de `test_box_tree`. Confirma la garantía "byte-idéntico" prometida en la spec del incremento 1. Ver `[[freedom-modernization-roadmap]]`, `spec/compositor.md`.
- **Fix (2026-07-16) — z-index negativo pintaba ENCIMA del contenido in-flow, no detrás.** Hallado al verificar visualmente el incremento 2: el comentario de `gui/browser_ui.c` ("Two passes: negative z_index paints BEHIND in-flow content") era falso — ambas pasadas de Stage 2 corrían DESPUÉS de pintar `L.boxes`/`L.rows`, así que una caja `position:absolute; z-index:-1` siempre terminaba encima. (Investigación paralela descartó un segundo sospechoso: la exclusión de flujo para `position:absolute/fixed` en `gui/browser_ui.c` **sí funciona** — confirmado con `content_h` idéntico con/sin la caja absoluta y con la misma fila de texto terminando en el mismo row tras compararlo con un control sin la caja.) Fix: extraído `paint_positioned_one` (antes duplicado entre las dos pasadas) y reordenado tanto `paint_structured` (pantalla) como `write_doc_png` (`--download-png`, headless) para pintar el prefijo de cajas `z_index<0` de `L.positioned` (ya contiguo gracias a `cx_sort`) **antes** del contenido in-flow, y el resto **después**, sin tocar `pdf_export` (que ya documentaba omitir z negativo como límite v1, no lo pintaba mal). Verificado con `git stash`: pixel de prueba `(255,0,0)` antes → `(0,0,255)` después en una página con una caja azul in-flow superpuesta a una caja roja `z-index:-1`. Test nuevo `test_download_png_negative_zindex_paints_behind_inflow` (`tests/test_freedom.c`) decodifica el PNG exportado con `image_decode` (el propio decoder del proyecto, no una lib externa nueva) y verifica el píxel — falla contra el código viejo, pasa contra el nuevo. `test_freedom` ahora enlaza `image_decode.o` (Makefile). Suite completa + `make asan` limpios (0 FAILED, 0 SUMMARY de sanitizer). Ver `[[freedom-compositor-foundation]]`.
- **Hito M1.1 (2026-07-16, incremento 3/N — opacity de grupo REAL, primer stacking-context con surface offscreen).** Cableado de extremo a extremo, `opacity` de CSS hasta el GUI: `pv_box_def.opacity` nuevo (`page_view.h`/`page_view.c` `boxdef_from_style`, gateado por caps.css como el resto del árbol de cajas vía `rd_build`) → IPC del worker (`tab.c` `write_view`/`read_view`, box de 58→**59** int32, campo `opacity` al final) → `box_tree.c` alimenta el valor real a `cx_style.opacity` en `bt_resolve_positioning` (antes hardcodeado a -1) → `gui/browser_ui.c` `paint_positioned_one`: cuando `opacity` está en `[0,99]`, `cairo_push_group`/`pop_group_to_source`/`paint_with_alpha` compone la decoración + contenido de la caja como **una sola unidad offscreen** antes de atenuarla — no cada trazo por separado (que doble-mezclaría donde fondo/borde/texto se solapan). Es el primer *stacking context* real del compositor con su propia surface, la pieza que `spec/compositor.md` dejaba fuera de alcance en el incremento 1. `--dump-dom` imprime `opacity=N` en la caja (`dom_debug.c`). **Verificado con medición de píxel exacta** (no solo visual): caja azul `#0000ff` opacity:0.5 sobre página blanca → píxel `(127,127,255)` (el blend correcto), confirmado con debugging directo de la surface Cairo cruda cuando un resultado inicial parecía "gris" (era el fondo amarillo de la página de prueba blendeando correctamente con el azul — no un bug; la caza de ese fantasma quedó documentada en memoria para no repetirla). Test nuevo `test_download_png_group_opacity_blends_with_background` (falla contra el código con la composición deshabilitada, pasa con ella activa) + extensión de `test_load_view_codec_full_roundtrip` (opacity sobrevive el roundtrip IPC completo por el worker). Suite completa + `make asan` limpios; página sin `opacity` (docs/index.html) sigue byte-idéntica. Ver `[[freedom-compositor-foundation]]`, `[[freedom-modernization-roadmap]]`.
- **Hito M1.1 (2026-07-16, incremento 4/N — opacity en cajas in-flow + `mix-blend-mode` + `isolation`, modo boy-scout).** Extiende el incremento 3 a cajas **in-flow** (no posicionadas) y a los dos triggers de `cx_forms_stacking_context` que faltaban: `pv_box_def.mix_blend`/`.isolation` nuevos (box IPC **59→61** int32), `box_tree.c` los alimenta a `cx_style` (ya no hardcodeados a 0). Helper único `box_forms_stacking_context` (single source of truth vía `cx_forms_stacking_context`) reemplaza el chequeo ad-hoc de solo-opacity en `paint_positioned_one`; `bui_blend_operator` mapea los 12 valores de `mix-blend-mode` 1:1 a operadores Cairo nativos (`CAIRO_OPERATOR_MULTIPLY`...`HSL_LUMINOSITY`) — verificado con aritmética exacta: `multiply(#ff8800, #0000ff) = negro puro`. **Hallazgo real durante la implementación** (no solo un bug de build): agrupar solo la decoración de una caja in-flow (fondo/borde) sin su(s) fila(s) de texto se veía **peor** que no arreglar nada — el fondo de la fila (`paint_content_row`, un draw call separado que cascadea el mismo color de fondo) seguía pintándose opaco encima de una caja correctamente atenuada, un rectángulo sólido flotando sobre un fondo translúcido. Fix real: `paint_box_and_direct_rows` agrupa la decoración de la caja **y** sus filas directas (`row_owner_block_id`) en un solo `push_group`, con un array `row_done` para que el loop de filas del llamador no las repinte. Esto reveló un segundo gap preexistente: `rc_row.blk` solo se puebla para filas `RC_IMAGE`; una fila de texto lleva su caja dueña en `rc_frag.block_id` (el primer fragmento), no en `row.blk` — el recorte `overflow:hidden` para texto plano llevaba tiempo resolviendo silenciosamente a "sin caja dueña" (nunca se activaba, solo para imágenes). `row_owner_block_id` (helper único) lo corrige en los 3 sitios que resolvían `row_bid` para `ov_reconcile`. Boy-scout DRY: el Stage 2 de `write_doc_pdf` (código duplicado ~60 líneas, una TERCERA copia del patrón de `paint_positioned_one`) ahora reusa `paint_positioned_one` directamente — de regalo, gana recorte `overflow:hidden` y compositing de grupo que el PDF nunca tuvo (v1 seguía omitiendo z-index negativo, límite ya documentado, no tocado). **Gotcha de build repetido**: crecer `pv_box_def` sin `make clean` produjo un valor de `mix_blend` corrupto (49) leído de un `.o` con el layout de struct desincronizado — mismo gotcha ya documentado para `css_style`, confirmado NO ser un bug de lógica tras rebuild limpio. Tests nuevos `test_download_png_inflow_opacity_blends_box_and_row_together` y `test_download_png_mix_blend_multiply_uses_cairo_operator` (ambos verificados: fallan con la fila/el blend deshabilitado, pasan con el código real) + extensión de `test_load_view_codec_full_roundtrip` (mix_blend/isolation sobreviven el roundtrip). Suite completa + `make asan` limpios; docs/index.html y todas las páginas sin estos triggers siguen byte-idénticas. **Fuera de alcance, explícito:** un árbol de capas anidado real (agrupar recursivamente cajas hijas dentro de su padre translúcido) — este incremento agrupa la decoración de una caja y solo sus filas DIRECTAS, no subárboles completos; ver `spec/compositor.md` §7. Ver `[[freedom-compositor-foundation]]`.
- **Hito M1.2 (2026-07-16) — CSS `transform` (2D translate only), primera propiedad de transformación real.** Continuación directa del roadmap aprobado (Fase 1: compositor → **transform** → animation → filter); alcance deliberadamente acotado a `translate()`/`translateX()`/`translateY()` en px (el valor de transform más común en sitios reales, p. ej. trucos de centrado `top:50%;transform:translateY(-50%)`), dejando `scale()`/`rotate()`/`skew()`/`matrix()` explícitamente fuera (necesitan matriz Cairo real + hit-testing transformado, no solo posición de pintado). Cadena completa: `expand_transform` en `css.c` (grámatica de función balanceada, mismo patrón que `linear-gradient()`; reusa `interp_len` — %, otras unidades, `scale()`/`rotate()`, múltiples funciones o sintaxis inválida fallan cerrado a "unset", nunca una transformación a medias) → `css_style.transform_tx/ty` (`CSS_LEN_UNSET` centinela) → `pv_box_def.transform_tx/ty` (`boxdef_from_style`) → IPC del worker (`tab.c`, box **61→63** int32) → `box_tree.c` alimenta `cx_style.has_transform` (nuevo campo en `compositor.h`, `cx_forms_stacking_context` ahora también dispara por transform) → `gui/browser_ui.c` `box_transform_offset` suma el offset SOLO a las coordenadas de pintado (`paint_box_decoration`/`paint_content_row`) en los 3 sitios de pintado (`paint_positioned_one`, `paint_box_decoration_grouped`, `paint_box_and_direct_rows`) — el rect propio de la caja (hit-testing, click dispatch, resolución de ancestros `overflow:hidden`) queda **sin tocar**, transformar el hit-test es trabajo futuro explícito. `--dump-dom` imprime `translate(tx,ty)`. **Bug real cazado por el test de parsing** (no un bug de build): `translateX(-15px)` emitía TAMBIÉN una declaración `P_TRANSFORM_TY=0` fantasma (ambos slots se emitían incondicionalmente), pisando un `translateY` separado o dejando `ty` en 0 en vez de `CSS_LEN_UNSET` — `expand_transform` ahora emite solo el/los slot(s) que la función realmente especifica. **Segundo hallazgo, más amplio:** `css_has_boxdeco` (el gate que decide si un elemento se registra como caja) no verificaba `opacity`/`mix_blend`/`isolation`/`transform` — un `<div style="opacity:.5">` SIN padding/borde/position nunca habría recibido entrada de caja y la propiedad habría fallado en silencio; corregido para las CUATRO propiedades a la vez (el mismo gate que hasta ahora solo miraba padding/border/radius/shadow/outline/position/visibility/overflow/cursor/etc.). Verificado con aritmética de píxel exacta (no solo visual): caja azul con `translate(200px,0)` → posición original blanca (sin pintar), posición trasladada azul pura. Tests nuevos: `test_inline_transform_translate` (parsing, 12 casos incluida la regresión de `translateX` cazada arriba) + `test_download_png_transform_translate_moves_paint_position` (verificado: falla con el offset deshabilitado, pasa con el código real) + extensión de `test_load_view_codec_full_roundtrip` + `test_sc_transform` en `test_compositor.c`. Suite completa + `make asan` limpios tras rebuild limpio (gotcha de `make clean` de nuevo evitado a propósito esta vez); docs/index.html y baselines previos (incrementos 2-4) siguen byte-idénticos. **Fuera de alcance:** `scale`/`rotate`/`skew`/`matrix`, hit-testing transformado (clic en una caja trasladada sigue resolviendo contra su rect ORIGINAL), `transform-origin`. Ver `[[freedom-compositor-foundation]]`, `spec/compositor.md`.
- **Hito M1.2b (2026-07-16) — CSS `transform`: `scale()`/`rotate()`, matriz Cairo afín real.** Continuación directa de M1.2 (el propio spec la dejó pendiente: "necesitan matriz Cairo real, no solo un offset aditivo"), elegida sobre las otras dos opciones abiertas (árbol de capas anidado, M1.3 animación) por ser la que más sitios reales usan hoy sin necesitar todavía un frame clock (`scale(1.05)` en hover, iconos rotados, spinners, badges). `expand_transform` en `css.c` gana 4 funciones nuevas (`scale`/`scaleX`/`scaleY`/`rotate`), manteniendo la restricción "una función por declaración" (encadenar `translate() rotate()` en un solo valor sigue fallando cerrado — ver `test_inline_transform_independent_cascade_combines` para el caso legítimo: dos REGLAS distintas sí pueden combinar translate+rotate porque cada función es un slot de cascada independiente, `P_TRANSFORM_SX/_SY/_ROTATE` nuevos). `scale`/`scaleX`/`scaleY` via `parse_scale_pct` (ratio unitless → percent-of-identity, mismo convenio que `font_scale`; `scale(sx)` sin segundo argumento significa `scale(sx,sx)`, NO `scale(sx,1)` — a diferencia de `translate`); `rotate` via `parse_rotate_deg` (grados enteros, sufijo `deg` obligatorio, mismo convenio que el ángulo de `linear-gradient`; `rad`/`turn`/`grad`/fraccionarios fallan cerrado). Cadena: `css_style.transform_sx/sy/rotate` (`CSS_LEN_UNSET` centinela) → `pv_box_def` (mismos nombres) → IPC (`tab.c`, box **63→66** int32) → `box_tree.c`/`gui/browser_ui.c` `box_forms_stacking_context` amplían `has_transform` a los 3 campos nuevos → **reemplazo completo** de `box_transform_offset` (el offset aditivo tx/ty de M1.2) por `box_transform_matrix` (nuevo, `cairo_matrix_t`: `cairo_matrix_translate`+`_rotate`+`_scale` pivotados en el CENTRO de la caja — `transform-origin` no se parsea, siempre 50%/50%) aplicada con `cairo_save`/`cairo_transform`/`cairo_restore` alrededor de cada llamada de pintado en los mismos 3 sitios de M1.2; unifica translate/scale/rotate bajo una sola matriz en vez de un caso especial aditivo. El *cull* de filas fuera-de-viewport (optimización, no un clip) sigue usando solo el componente de traslación vertical — deliberadamente ignora rotate/scale (documentado: en el peor caso pinta o salta una fila cerca del borde unos píxeles de más, el clip real del viewport sigue acotando visualmente). **Verificado con aritmética de píxel exacta** (no solo visual): página con caja ancha-y-baja rotada 90° pasa de "ancha y baja" a "angosta y alta" (un punto que estaba DENTRO del footprint original y AHORA está fuera pasa de azul a blanco, y viceversa — un simple offset aditivo nunca podría producir este cambio de FORMA); caja con `scale(1.5)` crece simétricamente alrededor de su centro (dos puntos justo fuera del borde original, arriba y abajo, pasan de blanco a azul). Revisión visual con 5 cajas (baseline/rotate/scale/cadena-ilegal/scale-no-uniforme) confirma pivote correcto y que el encadenado ilegal cae exactamente en la posición baseline (fail-closed real, no solo en el parser). Tests nuevos: `test_inline_transform_scale`/`test_inline_transform_rotate` (parsing, incluye `scale(sx)`≡`scale(sx,sx)`, `rotate` fail-closed en rad/turn/fraccionario/matrix/skew) + `test_inline_transform_independent_cascade_combines` (cascada independiente) + `test_download_png_transform_rotate_changes_paint_shape`/`test_download_png_transform_scale_grows_around_center` (verificados: fallan con la matriz deshabilitada, pasan con el código real) + extensión de `test_load_view_codec_full_roundtrip`. Suite completa (1219 tests) + `make asan` limpios tras `make clean` (gotcha de struct-growth de nuevo evitado a propósito). **Fuera de alcance, sin cambios:** `skew`/`matrix`, encadenado de múltiples funciones, `transform-origin`, hit-testing transformado. Ver `[[freedom-compositor-foundation]]`, `spec/compositor.md`.
- **Hito (2026-07-16) — Imágenes `data:` inline + fallback `srcset`, cierre del gap #1 de renderizado de imágenes.** Nuevo módulo puro `data_url` (`du_`, fuzzeado 17M execs limpio): `du_is_data_url`/`du_base64_payload` (recorte puro, sin decodificar) + `du_base64_decode` (RFC 4648, propio, sin OpenSSL — antes TODO `<img src="data:...">` caía a `RDP_IMG_BLOCK_SCHEME` porque `rp_evaluate` exige https; un patrón altísimo en sitios modernos, íconos/logos/spinners inline). `rdp_image_decision` salta el chequeo de scheme/host/tracker para `data:` (no hay socket, no hay IP que filtrar) pero sigue exigiendo `caps.images`. Decode confinado de extremo a extremo: el padre solo recorta el substring base64 (aritmética de punteros, mismo nivel de confianza que `url_resolve_https` ya parseando URLs hostiles ahí); el worker decodifica base64 Y formato — nuevo opcode IPC `OP_DECODE_IMAGE_B64` reusando `child_handle_decode_image` (DRY), `tab_decode_image_data_url` en el padre. **Boy-scout, cerró el residual "image (allowed)" sin razón:** el guard `cur_top == NULL` en `load_images` ahora exime a `data:` (no necesita origen, igual que en `render_doc`). **Fallback `srcset`** en `page_view.c` (`srcset_first_url`, pura): un `<img>` sin `src` (patrón común de imágenes responsivas y librerías de lazy-loading) usa el primer candidato de `srcset`; un candidato `data:` se escanea hasta el primer espacio o fin de cadena (no la primera coma, que sería la de `;base64,`) para no truncarlo. **Bug real cazado durante el testing, no de mi código:** el string-meme "PNG 1x1 más pequeño" que circula en tutoriales tiene el CRC del IDAT roto — Pillow lo acepta (no verifica CRC), libpng estricto (ya en uso) lo rechaza correctamente; confirmó que el pipeline está siendo apropiadamente estricto, no que hubiera un bug de wiring. Verificado con PNG headless real (ícono inline decodifica y pinta; `<picture>`+`srcset`-only renderizan). 22 tests `data_url` + 5 tests `page_view` (srcset) + 6 tests `render_doc`/`render_policy` (decisión) + 3 tests `tab` (worker) nuevos; suite completa + `make asan` limpios. Ver `[[freedom-data-url-images]]`, `spec/data_url.md`.
- **Hito (2026-07-16) — CSS `background-image: url()` real + `background-size`/`background-repeat` pintados, cierre del gap de mayor impacto visual (héroes/íconos/patrones de sitios modernos).** Hasta ahora `url()` en `background-image`/`background` era "dropped by doctrine" (nunca capturado); esta doctrina ya no aplicaba una vez que `<img>` fetchea bajo política gateada (Hito data-url) — un `background-image` es la MISMA operación de confianza, así que se cablea igual, no se relaja nada. `css.c`: `expand_bg_image`/`expand_background` capturan `url(...)` (citado o no, 1 sola capa; multi-capa/junk fallan cerrado) en un pool pequeño por-parseo (`bg_urls[256]` a nivel de sheet, tabla stack-local para inline — `css_decl` sigue siendo solo-int, el pool evita inflar el array de 32768 decls); nuevo slot `P_BG_IMAGE_URL`. **Boy-scout, cerró un bug real de anti-DoS más amplio:** el buffer de VALOR de una declaración (`val`/`resolved` en `parse_one_decl`) compartía el mismo `CSS_TOK_MAX` (64 bytes) que los tokens de SELECTOR — cualquier valor legítimo más largo que ~64 caracteres (un `box-shadow` largo, un `linear-gradient` con muchas paradas, y ahora cualquier URL de CDN con hash/querystring) se descartaba en silencio; nuevo `CSS_URL_MAX` (1024, dimensionado para contener también la URL absoluta YA RESUELTA) reemplaza esos buffers específicos, dejando `CSS_TOK_MAX` intacto para selectores. Cadena completa: `css_style.bg_image_url` (cruda, sin resolver) → `pv_box_def.bg_image_url`/`bg_size`/`bg_repeat` (`page_view.c`, mismo gate `css_has_boxdeco` que opacity/transform) → IPC del worker (`tab.c`, box **68** int32 + un `write_field`/`read_field` nuevo para la URL, igual que los strings de un run) → `render_doc.c` (`resolve_image_decision`, extraído de DRY del case `PV_IMAGE` y reusado para ambos: resuelve contra el origen y aplica **la misma política exacta que un `<img>`** — `caps.images`, `rdp_image_decision`, confinamiento `file://`, `data:` sin red — cualquier resultado que no sea `RDP_IMG_ALLOW` limpia el campo, fail-closed, el GUI nunca ve una URL bloqueada) → `gui/browser_ui.c` (`fetch_decode_image` extraído de DRY de `load_images`, reusado por el nuevo `load_bg_images`; pintado en `paint_box_decoration` vía Cairo pattern con `cairo_matrix` para `cover`/`contain`/tamaño natural y clip-a-franja para `repeat-x`/`repeat-y` ya que Cairo solo tiene un `EXTEND_REPEAT` que tila ambos ejes). Pinta ENCIMA de `bg_rgb`/gradiente y DEBAJO del borde (orden de capas real de CSS). `background-position` sigue sin resolverse (v1 pinta siempre desde la esquina superior izquierda, que además ES el valor inicial de CSS `0% 0%`, no un compromiso). El toast de privacidad de imágenes ahora también dispara por background-image cuando `caps.images` está off. Verificado con PNG headless real (no solo tests): `cover`/`contain`/`no-repeat` con el logo del proyecto, y `repeat-x`/`repeat-y`/`repeat` con una textura 2×2 de colores mostrando tiling correcto por eje; fail-closed verificado visualmente contra `javascript:` y `http://` no-https (caja plana, sin fetch, sin crash). 8 tests `css` nuevos + extensión de `test_load_view_codec_full_roundtrip` (IPC) + fix de un test existente que dependía del límite viejo de 64 bytes (`test_container_fail_closed_and_bounds`, adaptado al nuevo `CSS_URL_MAX`); suite completa + `make asan` limpios tras `make clean`. Ver `[[freedom-css-background-image]]`, `spec/css.md`.

- **Hito (2026-07-17) — Pipeline de video para jkanime.net (iframe->.m3u8/.mp4).** `try_create_iframe_from_script` extrae TODOS `video[N]` (<=8, no solo 1) para fallback entre servidores; `jd_process_iframes` remplaza estado static por `jd_opaque.iframe_track` (per-context, sin leak); `scan_m3u8_url` extendido a `scan_video_url` con fallback `.mp4`. 6 tests BDD para patrones jkanime (video[0..4], variable-to-variable, relativa, literal, no-video, null). ASan limpio. `config/allow.conf`: +`fonts.googleapis.com`, `fonts.gstatic.com`. Ver `spec/video_pipeline_jkanime.md`, `[[freedom-jkanime-video-pipeline]]`.
