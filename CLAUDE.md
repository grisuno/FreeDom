...
medición (ago 13, tanda 18, `TOTAL 154.06` sobre **15** páginas — el corpus viejo quedó plano en
147.72 y la 15.ª es la sonda nueva): `jkanime` 53.12, `wikipedia` 40.02, `ddg-results` 18.19,
`slashdot` 15.05, `hackernews` 12.59, `em-units` **6.33**, `table-fit` 1.64, `float-beside` 1.23,
`inline-replaced` 1.15, `ua-metrics` 0.98, `inline-links` 0.92, `rem-62` 0.89, `css-table` 0.78,
`pct-box` 0.75, `multicol` 0.43. El término dominante del score es
`h_ratio`, así que **lo que apila verticalmente lo que debería ir al lado es siempre el bug más
caro.**

**Brecha nueva, medida y sin arreglar (ago 13):** `margin-bottom` (y todo margen
vertical) se descarta **entero** en un bloque cuyo contenido es solo una caja,
sin runs de texto. Repro de tres líneas: tres `<div>` de 20 px con
`margin-bottom:30px` salen en 0/20/40 contra **0/50/100** de Firefox. Con texto
adentro los mismos márgenes funcionan (0/53/106 = Firefox exacto), así que el
bug está en el apilado de cajas sin runs (`nbox=3 nrow=0`), no en la cascada.
Afecta a todo `<div>` decorado vacío, separador, spacer y contenedor de ícono,
o sea a toda página real. Es el próximo objetivo sistémico.
...
| R5 | `grid-template-rows`, `grid-row: span N`, y **colocación por nombre/línea** (`grid-template-areas`, `grid-area`, `grid-*-start/-end`, `justify-self`): hoy los ítems van round-robin `índice % cols`. Es el residual medido de wikipedia y jkanime. | Sin empezar | **Alto valor** |
