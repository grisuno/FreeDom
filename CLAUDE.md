...
| :-- | :-- | :-- | :-- |
| R3 | **Tablas: ancho automático (shrink-to-fit).** **Ya hecho (ago 2):** la tabla de datos hace grid de N columnas (celdas lado a lado) en flujo plano **y dentro de columnas de float** — antes colapsaba a lista vertical de 1 columna. **Falta:** columnas shrink-to-fit (hoy reparten el ancho en partes iguales, no por contenido) y `colspan`/`rowspan` reales en el motor de layout (el atributo se lee pero cada celda ocupa una celda). | Parcial | Medio |
