...
medición (ago 11, `TOTAL 149.95`): `jkanime` 53.81, `wikipedia` 44.00, `ddg-results` 19.74,
`slashdot` 14.91, `hackernews` 12.75, `table-fit` 1.64, `float-beside` 1.23, `ua-metrics` 0.98,
`rem-62` 0.89. El término dominante del score es `h_ratio`, así que **lo que apila verticalmente
lo que debería ir al lado es siempre el bug más caro.**

**Lección de la tanda 13, vale para elegir el próximo objetivo:** los cuatro bugs más caros no
estaban en ninguna página en particular — eran **constantes y unidades del motor** que se
componen con todo lo demás. Antes de perseguir el layout de un sitio, comprobá si la brecha es
un factor uniforme: se mide en un probe de 10 líneas y suele valer más que semanas de casos.

| # | Hito | Estado | Esfuerzo |
| :-- | :-- | :-- | :-- |
| R12 | **`jkanime` (53.8) y `wikipedia` (44.0) siguen siendo las dos peores, y ahora son las ÚNICAS por encima de 20.** Ambas son el mismo bug de fondo: **un contenedor flex reparte ~0 px a un ítem que debe crecer**, y el texto cae a una línea por carácter. **MEDIDO (ago 11):** en Wikipedia el `<h1>` del título (`flex-grow:1`, `justify-content:end`) recibe **1 px** y "Web browser" sale en 10 filas verticales de 79.5 px; su hermano es un menú de idiomas `position:absolute; visibility:hidden` que **no debería contribuir** al sizing flex. Aislado en una sonda de 8 líneas **NO reproduce** (el título toma 743 px), así que la causa está en una cadena más larga — ese es el próximo paso: bisecar desde el HTML real hacia el probe, no al revés. **Ya no es** el layout fijado de `@media` (tanda 12) ni el pie de foto de 15 px (tanda 13, `::before` — 131 filas → 0). Sigue faltando: placement por `grid-template-areas` (los ítems van round-robin `índice % cols`) y CSS multi-columna (`column-width:30em` cae a 1 columna). **jkanime** es Bootstrap **flex**: `col_mae` 0.31 es el término dominante allí, no el alto — la barra "ANIMES RECIENTES" apila ~40 tarjetas media-object en vez de miniatura+texto al lado, y las imágenes remotas rotas offline inflan la brecha con su texto `alt` (que en Firefox no envuelve igual), así que **parte de su score no es un bug del motor**. `[[freedom-vertical-metrics-and-pseudo-leak-2026-08-11]]`, `[[freedom-parity-container-diagnosis-2026-08-10]]` | Parcial | Alto |
