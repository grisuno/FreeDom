...
| :-- | :-- | :-- | :-- |
| R12 | **`jkanime` (4.12×) y `wikipedia` (4.49×) son ahora las dos peores, y las dos por el camino de CONTENEDORES, no por floats.** Wikipedia: 2435 de 2601 bloques van en contenedor flex/grid (skin Vector), el render sale con bandas vacías enormes, el `<h1>` partido en dos líneas gigantes y ~1103 filas de línea donde Firefox necesita ~270 — o sea las líneas salen mucho más angostas de lo que deberían. Sin causa única identificada: **medir primero** (`--dump-layout` + gaps por fila) y no suponer. | Sin empezar | Alto |
