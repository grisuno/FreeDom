...
- **Paridad Firefox tanda 3 (jul 31) — tipografía y flujo inline.** `font-size` absoluto (`font_abs`) reemplaza la escala UA vs relativo que encadena; métricas UA alineadas con el navegador (base 16px, `heading_scale` 2.0…0.67, títulos heredan color); `<h1>` des-negritable vía peso resuelto; colapso de espacio en el borde entre runs sin inventar; `position:absolute|fixed`/`float` coercen a bloque; el PNG se dimensiona por la caja más baja. `[[freedom-typography-parity-2026-07-31]]`
- **Paridad Firefox tanda 4 (jul 31) — estructura: anidamiento y flujo inline.** Contenedores flex/grid **ANIDADOS** vía tabla `pv_cont_def` (cruza el IPC como arreglo; `layout_container` recursiona sobre ítems-contenedor) y `display:inline-block` que fluye DENTRO de la línea (caja de nivel inline materializada en `flush_line`). Contrato: caja raíz de ítem anidado = la del contenedor hijo; contenedor sin caja hereda la primera de afuera (`box_pending`); los contenedores sintetizados llenan el descriptor. `spec/page_view.md`, `[[freedom-nested-flex-containers-design]]`
- **Paridad Firefox tanda 5 (jul 31) — grid de Bootstrap: runs reemplazados + anidamiento profundo.** Toda página `.row/.col` (jkanime) colapsaba a una columna estrecha. Cinco brechas que se sumaban, medidas contra Firefox (2588px apilado → 830px horizontal): (1) un ítem flex con contenido mixto `[texto, .row anidado, texto]` se decidía por su PRIMER run y se fluía plano → el anidado se apilaba; ahora `layout_container` **particiona el ítem en segmentos** por `child_cont_at_level` y recursiona por segmento. (2) las props flex del **wrapper** de un contenedor anidado (`.col{flex:1 0 0%}` envolviendo un `.row`) no viajaban — el flat model solo estampa flex por-run relativo al contenedor más interno; nuevos campos `pv_cont_def.item_{grow,shrink,basis,order,align_self}` capturados en el walk (`anc_item_*[]`) e hilvanados por el códec. (3) **la causa dominante:** los runs reemplazados (`<input>`/`<select>`/`<img>`/`<svg>`) perdían la membresía flex en LOS TRES cruces del códec (emisión en page_view, copia en `rd_build` gateada a TEXT/LINK/VIDEO, `read_view` bajo `if(kind!=PV_INPUT)`) → `cont_id=-1` **rompe la corrida del contenedor** en el bucle de nivel superior. (4) `emit_replaced_row` extraído y compartido por el flujo plano y el de ítem flex (antes una `<img>` en columna era placeholder de ancho 0 y desaparecía). (5) el painter pintaba RC_IMAGE/VIDEO/INPUT en `left`+ancho completo ignorando `x_off`/ancho de columna → solape; ahora `left+x_off` y `bg_w`. `[[freedom-flex-replaced-runs-and-nesting-2026-07-31]]`
