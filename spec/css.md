...
| `flex` shorthand | sets `flex_grow`/`flex_shrink`/`flex_basis`: `flex: N` → N 1 0; `none` → 0 0 auto; `auto` → 1 1 auto; `initial` → 0 1 auto; up to three explicit values in order grow shrink basis. Per CSS Flexbox §7.1.1 the third component is a `<'flex-basis'>`, and a **unitless zero is a valid `<length>`** there, so `flex: 1 1 0` (a very common idiom) means basis 0 — not a third factor. A unitless NON-zero in that position is not a length and fails the whole declaration closed (no invented unit), matching `flex-basis` itself. |
...
| :-- | :-- | :-- |
| `visibility` | `visibility` (`css_visibility`): `visible`/`hidden`/`collapse`; `collapse` (meant for table rows) has no real table-row model here, so it collapses to `hidden` | **Yes.** `hidden` skips painting the box/its content but keeps its layout space (unlike `display:none`, which removes it entirely). **It INHERITS (CSS 2.1 §11.2)** and is resolved per-run by `page_view` from the nearest ancestor that sets it, so text with no box of its own (a bare `<span>`) hides too, and a descendant that declares `visibility:visible` re-appears inside a hidden subtree. The pure rule is `pv_content_hidden(box_hidden, run_visibility)`: an explicit value on the run beats the enclosing box stack in both directions. Gating is per-FRAGMENT; a text row short-circuits only when every fragment is hidden. A nested container (flex/grid) inside a hidden box is a known v1 gap — see below. |
