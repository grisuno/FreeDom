 ...
 * length (no children => an owned empty string). Uses a chain of fixed-size
 * blocks internally so there is no hard cap — OOM from malloc is the only limit.
 * Invalid handle / NULL out => DOM_ERR_NULL_ARG. */
