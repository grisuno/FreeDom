...
void pv_set_grid_area(pv_view *v, int row_start, int col_start) {
    if (v == NULL || v->count == 0) return;
    pv_run *r = &v->runs[v->count - 1];
    r->grid_row_start = row_start;
    r->grid_col_start = col_start;
}
...
/* Anti-DoS bound on the BOX registry (spec/page_view.md). Distinct from
 * PV_MAX_CONTAINERS: a flex/grid container is a rare element, while a box is
 * generated by anything with a border, a background, padding or a vertical margin,
 * which on a real page is most of the document. Sized so no ordinary page is
 * truncated -- the front page of a news site registers a few hundred -- while a
 * page built to exhaust memory still stops. */
#define PV_MAX_BOXES        1024u

/* The box registry and the out-of-flow solver index the same ids, so a box the
 * registry accepts must be one the solver can address (include/box_tree.h). */
_Static_assert(PV_MAX_BOXES <= BT_MAX_POSITIONED,
               "PV_MAX_BOXES must fit the out-of-flow solver's per-box arrays");
 ...
 * Resolved on the trusted side from grid-template-areas + grid-area, so the
 * painter receives numbers and never a name. */
...
static int css_has_boxdeco(const css_style *cs) {
    ...
    return cs->pad_top > 0 || cs->pad_right > 0 ||
           ...
           /* A non-zero VERTICAL MARGIN alone makes the box worth tracking. A margin
            * is space OUTSIDE the border box, so the only place the painter can
            * apply it is where that element's box begins -- and without a box entry
            * it had nowhere to live. The engine only ever carried the NEAREST block
            * ancestor's margins on the text run, so an undecorated wrapper
            * (`section { margin-bottom: 24px }` around a couple of <div>s, or any
            * `.row`/`.section`/`.stack` class whose whole job is vertical rhythm)
            * lost its margins entirely -- and a page whose blocks are all decorated
            * boxes had no run to carry anything at all. Zero and `auto` are not
            * margins for this purpose: `margin: 0` is the universal reset and
            * registering a box for it would make the painter flush a line at every
            * element that ships one. Negative counts -- overlapping borders with
            * `margin-top:-1px` is exactly a case that must survive. */
           (cs->margin_top != CSS_LEN_UNSET && cs->margin_top != CSS_LEN_AUTO &&
            cs->margin_top != 0) ||
...
/* The registry grows on the HEAP, doubling, up to PV_MAX_BOXES.
 *
 * It used to be two fixed arrays inline in the struct, which put 1.5 MiB of
 * pv_box_def on pv_build's stack frame AND capped a page at 256 boxes -- a bound
 * a real page passes without trying (slashdot's front page saturates it), and past
 * it box_reg_id returns -1, so an element silently loses its box: no decoration, no
 * margins, and its descendants lay out against the page instead of against it. A
 * limit that a normal page hits is not an anti-DoS bound, it is a rendering bug.
 * PV_MAX_BOXES is the real bound and it is still hard: past it the page renders
 * with the boxes it has rather than growing without limit.
 *
 * Returns 0 on success, -1 when the cap or memory is exhausted (the caller then
 * degrades to "no box for this element", never a crash). */
static int box_reg_grow(pv_box_reg *r) {
    size_t ncap = (r->cap == 0) ? 64u : r->cap * 2u;
    if (ncap > PV_MAX_BOXES) ncap = PV_MAX_BOXES;
    if (ncap <= r->cap) return -1;
    const lxb_dom_node_t **nn =
        (const lxb_dom_node_t **)realloc(r->node, ncap * sizeof *nn);
    if (nn == NULL) return -1;
    r->node = nn;
    pv_box_def *nd = (pv_box_def *)realloc(r->def, ncap * sizeof *nd);
    if (nd == NULL) return -1;
    r->def = nd;
    /* V-002: realloc leaves the new tail uninitialised, and a pv_box_def read
     * before boxdef_from_style fills it would make the render depend on heap
     * residue. Zero it here so a slot is never live before it is defined. */
    memset(r->node + r->cap, 0, (ncap - r->cap) * sizeof *nn);
    memset(r->def  + r->cap, 0, (ncap - r->cap) * sizeof *nd);
    r->cap = ncap;
    return 0;
}

static void box_reg_free(pv_box_reg *r) {
    free(r->node); free(r->def);
    r->node = NULL; r->def = NULL;
    r->count = r->cap = 0;
}
...
static void boxdef_from_style(pv_box_def *d, const css_style *cs) {
     ...
     * meet its neighbour's before either is used up (CSS 2.1 8.3.1 collapsing).
     * ua_tag is stamped by the caller, which is where the element's tag is known. */
    d->box_mt = (cs->margin_top    != CSS_LEN_UNSET) ? cs->margin_top    : PV_LEN_UNSET;
    d->box_mb = (cs->margin_bottom != CSS_LEN_UNSET) ? cs->margin_bottom : PV_LEN_UNSET;
    d->box_mt_pct = cs->pct[CSS_PCT_MARGIN_TOP];
    d->box_mb_pct = cs->pct[CSS_PCT_MARGIN_BOTTOM];
    d->ua_tag = BX_UA_NONE;
    d->font_px = 0;
...
/* Registers (or finds) the box for `node`. font_px is the element's COMPUTED
 * font-size, needed because the user-agent margins this box may fall back on are
 * expressed in em; <= 0 means the caller could not resolve it and the painter uses
 * the theme's body size instead. */
static int box_reg_id(pv_box_reg *r, const lxb_dom_node_t *node, const css_style *cs,
    ...
    /* The user-agent identity of the element behind the box, so the painter can ask
     * bx_block_ua_box for the margin the UA sheet gives it when the author declares
     * none -- the same question, answered from the same table, that a text run
     * already carries as pv_run.ua_tag. A non-element node keeps BX_UA_NONE (zero
     * UA margin), which is also the answer for div/section/td and every unknown tag. */
    if (node != NULL && node->type == LXB_DOM_NODE_TYPE_ELEMENT) {
        lxb_dom_element_t *bel = lxb_dom_interface_element((lxb_dom_node_t *)node);
        r->def[id].ua_tag = (int)ua_tag_of(lxb_dom_element_tag_id(bel));
    }
...
/* The COMPUTED font-size memoized for `node`, or 0 when it is not in the cache
 * (no cache, or the element's style was never resolved). Every caller that needs
 * it has already gone through cached_element_style for the same node, so this is
 * a lookup, never a second cascade. */
static double pv_cached_font_px(const pv_style_cache *cache, const lxb_dom_node_t *node) {
    long h = pv_cache_find(cache, node);
    return (h >= 0) ? cache->font_size[h] : 0.0;
}
    ...
    *block_id_out = -1;
    ...
    for (const lxb_dom_node_t *p = start; p != NULL; p = p->parent) {
        if (p->type == LXB_DOM_NODE_TYPE_ELEMENT) {
            ...
            /* Is THIS ancestor the heading itself? (Distinct from got_heading, which
             * only says the walk has PASSED one.) The font-size rule below needs the
             * difference. */
            int heading_here = 0;
            if (!got_heading) {
            ...
            if (box_reg != NULL && generates_box_style(t, &cs) && css_has_boxdeco(&cs)
                ...
                if (bid >= 0) {
                    if (!got_boxdeco) {
                        *block_id_out = bid;
                        got_boxdeco = 1;
                        /* A margin belongs to an ELEMENT and is applied exactly once,
                         * where that element's box begins -- OUTSIDE its border box.
                         * When this very ancestor is both the run's nearest block and
                         * the owner of the run's box, the box def already carries these
                         * margins (boxdef_from_style) and the painter applies them when
                         * it opens the box. Leaving them on the run too would apply the
                         * same margin a second time INSIDE the box, which is where the
                         * old engine put a block's whole margin-top: it made every
                         * decorated block that much taller instead of moving it down. */
                        if (block_set_here && box != NULL) {
                            box->mt = 0; box->mb = 0;
            ...
            if (!got_fs && cs.font_scale != 0) {
                if (cs.font_abs) {
                    *font_scale = round_pct(cs.font_scale * fs_rel);
                     ...
                     * only when the author declared it ON the heading (or inside
                     * it): that is a declaration for the same property on the same
                     * element, and a real cascade lets it win. Declared on an
                     * ANCESTOR it is merely the heading's INHERITED value, and the
                     * UA rule `h1 { font-size: 2em }` still doubles it -- so it must
                     * stay a multiplier of the UA base, not replace it.
                     *
                     * Treating every absolute size as a replacement made
                     * `body { font-size: 16px }` -- which is on essentially every
                     * real page, and is what the `font` shorthand expands to --
                     * render h1, h2 and p at exactly the same size: the whole
                     * document lost its heading hierarchy. The multiplier is exact
                        ...
                        if (is_real) {
                            ...
                            if (cs.display == CSS_DISP_GRID) {
                                ...
                                cd->grid_flow = (cs.grid_auto_flow != CSS_GF_UNSET)
                                                ? cs.grid_auto_flow : 0;
                                /* grid-template-areas defines the EXPLICIT grid just
                                 * as grid-template-columns/-rows do (CSS Grid 1 7.3):
                                 * a template with more columns than the track list
                                 * creates the missing ones. Taking the larger of the
                                 * two is what makes `grid-template-areas` work on its
                                 * own, which is how it is usually written. */
                                fx_area_map am;
                                if (fx_grid_areas_parse(cs.grid_areas, &am) == FX_OK) {
                                    if (am.cols > cd->cols) cd->cols = am.cols;
                                    if (am.rows > cd->rows) cd->rows = am.rows;
                                }
                            }
                        ...
                        if (!got_cont && cont != NULL) {
                            ...
                            if (have_prev_el) {
                                ...
                                if (is_real) {
                                    ...
                                    /* Named placement (CSS Grid 1 8.4): the item's
                                     * `grid-area: <name>` resolved against THIS
                                     * container's template. Resolved here, on the
                                     * trusted side, so only the numbers cross the IPC
                                     * codec -- the painter never sees a name. A name
                                     * the template does not hold leaves the item on
                                     * auto-placement, exactly as a browser does. */
                                    fx_area_map am;
                                    if (prev_area_name != 0u && cs.grid_areas[0] != '\0' &&
                                        fx_grid_areas_parse(cs.grid_areas, &am) == FX_OK) {
                                        int ar, ac, ars, acs;
                                        if (fx_grid_area_rect(&am, prev_area_name,
                                                              &ar, &ac, &ars, &acs) == FX_OK) {
                                            cont->grid_row_start = ar;
                                            cont->grid_col_start = ac;
                                            cont->col_span = acs;
                                            cont->row_span = ars;
                                        }
                                    }
...
pv_status pv_build_styled(const hp_document *doc, int js_enabled, int reader,
    ...
    *out = NULL;
    ...
    pv_box_reg box_reg = { NULL, NULL, 0, 0 };  /* box-carrying blocks + their defs (heap, grows) */
        ...
        if (n->parent != NULL && n->parent->type == LXB_DOM_NODE_TYPE_ELEMENT) {
            ...
            if (pcs.content_str[0] != '\0') {
                ...
                if (fresh) {
                    ...
                    /* ::before content is the FIRST INLINE CONTENT of its element
                     * (CSS 2.1 12.1), not a block of its own: the marker and the text
                     * it marks share a line. So the generated run takes the block
                     * break -- it is what opens the element's first line -- and the
                     * element's own text must NOT take another, which is what
                     * `prev_block = block` below says. Emitting it as a standalone
                     * block gave every generated marker a row of its own; on
                     * Wikipedia's reference list, where each entry is prefixed by a
                     * `^` backlink marker, that doubled the height of the whole
                     * section. It must also ride the SAME box, indent, container and
                     * text style as the element's text, or the painter reconciles
